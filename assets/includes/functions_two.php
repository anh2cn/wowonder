...
function Wo_ConfirmUser($user_id, $code) {
    ...
    if ($result == 1) {
        $query_two = mysqli_query($sqlConnect, " UPDATE " . T_USERS . "  SET `active` = '1' WHERE `user_id` = '{$user_id}' ");
        if ($query_two) {
            return true;
        }
    } else {
...
//print_r(Wo_PayPal('week', 'upgrade'));
...
function Wo_GetAvUpgrades($user_id) {
    ...
    $hot_member    = $wo['lang']['hot_member'] . ' ' . $wo['config']['monthly_price'] . '$ ' . $wo['lang']['per_month'];
    $ultima_member = $wo['lang']['ultima_member'] . ' ' . $wo['config']['yearly_price'] . '$ ' . $wo['lang']['per_year'];
    $vip_member    = $wo['lang']['vip_member'] . ' ' . $wo['config']['lifetime_price'] . '$ ' . $wo['lang']['life_time'];
    if ($user['pro_type'] == 1) {
        $can_see = array(
            'month' => $hot_member,
            'year' => $ultima_member,
