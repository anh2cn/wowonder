...
function Wo_GetUserFromSessionID($session_id, $platform = 'web') {
    ...
    $query      = mysqli_query($sqlConnect, "SELECT `user_id` FROM " . T_APP_SESSIONS . " WHERE `session_id` = '{$session_id}' AND `platform` = '{$platform}'");
...
function Wo_GetBlockedMembers($user_id) {
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_ImportImageFromUrl($media, $custom_name = '_url_image') {
    ...
    $fileget           = file_get_contents(str_replace(' ', '%20', $media), false, stream_context_create($arrContextOptions));
    if (!empty($fileget)) {
...
function Wo_RegisterMessage($ms_data = array()) {
    ...
    if (!isset($ms_data['stickers']) || !Wo_IsUrl($ms_data['stickers'])) {
...
function Wo_ShareFile($data = array(), $type = 0) {
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
            ...
            if ($type == 1) {
                ...
                if (Wo_Resize_Crop_Image(400, 400, $filename, $last_file, 60)) {
                    if ($wo['config']['amazone_s3'] == 1 && !empty($last_file)) {
                        $upload_s3 = Wo_UploadToS3($last_file);
                    }
                }
            } else {
        ...
        if ($wo['config']['amazone_s3'] == 1 && !empty($filename)) {
            $upload_s3 = Wo_UploadToS3($filename);
        }
        $last_data             = array();
...
function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
    ...
    if ((!isset($data['publisher_id']) || $data['publisher_id'] == $user)) {
...
function Wo_DeletePost($post_id = 0) {
    ...
    $user_id = Wo_Secure($wo['user']['user_id']);
    $post_id = Wo_Secure($post_id);
    $query   = mysqli_query($sqlConnect, "SELECT `id`, `user_id`, `recipient_id`, `page_id`, `postFile`, `postType`, `postLinkImage`, `multi_image`, `album_name` FROM " . T_POSTS . " WHERE `id` = {$post_id} AND (`user_id` = {$user_id} OR `recipient_id` = {$user_id} OR `page_id` IN (SELECT `page_id` FROM " . T_PAGES . " WHERE `user_id` = {$user_id}) OR `group_id` IN (SELECT `id` FROM " . T_GROUPS . " WHERE `user_id` = {$user_id}) OR `page_id` IN (SELECT `page_id` FROM " . T_PAGE_ADMINS . " WHERE `user_id` = {$user_id}))");
