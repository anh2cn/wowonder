...
function Wo_EventData($id = false) {
    ...
    if (!empty($fetched_data)) {
        ...
        $fetched_data['start_date'] = date($wo['config']['date_style'], strtotime($fetched_data['start_date']));
...
function Wo_GetStroies($args = array()) {
    ...
    $sql   = "SELECT * FROM " . T_USER_STORY . " WHERE `id` > 0 {$query_one} ORDER BY `id` DESC LIMIT 1";
...
function Wo_GetUsersByName($name = '', $friends = false, $limit = 0) {
...
function Wo_SharePost($id = false) {
    ...
    if ($post && mysqli_num_rows($post) > 0) {
        ...
        if ($query1 && $query2 && Wo_HidePost($id)) {
...
function Wo_GetPageAdmins($page_id = false) {
...
function Wo_PageRating($page_id = false) {
...
function Wo_GetNearbyUsers($args = array()) {
    ...
    HAVING distance < '$distance'   ORDER BY `user_id` DESC LIMIT 0, 20 ";
...
function Wo_GetFriendsStatus($data_array = array('limit' => 8, 'user_id' => 0)) {
    ...
    $query     = "SELECT * FROM " . T_USER_STORY . " WHERE (user_id IN (SELECT following_id FROM " . T_FOLLOWERS . " WHERE follower_id = '$user_id') OR user_id = $user_id) AND user_id IN (SELECT user_id FROM " . T_USERS . " WHERE active = '1') ORDER BY id DESC LIMIT " . $data_array['limit'];
    ...
    while ($fetched_data = mysqli_fetch_assoc($query_run)) {
        ...
        if (empty($fetched_data['thumbnail'])) {
            ...
            if (empty($fetched_data['thumb']['filename'])) {
